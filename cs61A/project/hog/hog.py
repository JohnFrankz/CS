...
def is_always_roll(strategy, goal=GOAL):
    ...
    False
    """
    for i in list(range(goal)):
        for j in list(range(goal - 1)):
...
def make_averaged(original_function, total_samples=1000):
    ...
    def compute(*args):
        score = 0
        for _ in list(range(total_samples)):
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    update_score = score + tail_points(opponent_score)
    if perfect_square(update_score):  
        update_score = next_perfect_square(update_score)
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    averaged_score_6 = make_averaged(roll_dice, 100)(6, six_sided)
    score_diff = GOAL - score
    if tail_points(opponent_score) >= score_diff or tail_points(opponent_score) >= averaged_score_6:
        return 0
    for i in list(range(1, 6)):
        score_of_roll_i = make_averaged(roll_dice, 100)(i, six_sided)
        if score_of_roll_i >= score_diff or score_of_roll_i >= averaged_score_6 :
            return i
    return 6  
